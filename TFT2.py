"""Second-generation modular dashboard controller.

TFT2 keeps scheduling, rendering, and data collection separate. The page layout
is configured in config.py and can later be switched by GPIO buttons without
...
def clear_screen():
    ...
    try:
        ...
        logging.getLogger("TFT2").exception("Failed to clear display")


def shutdown_handler(signum, frame):
    global running
    logging.getLogger("TFT2").info("Shutdown signal received: %s", signum)
    running = False
...
def main():
    ...
    logger.info("Starting TFT2 modular dashboard")
